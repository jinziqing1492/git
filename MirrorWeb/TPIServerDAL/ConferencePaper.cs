...
{
    public class ConferencePaper : IConferencePaper
    {
        private static string TABLE_NAME = ConfigurationManager.AppSettings["ConferencePaper"];
        #region IArticle 字段
        private const string PARM_CONFERENCENAME = "CONFERENCENAME";
        private const string PARM_CONFERENCETIME = "CONFERENCETIME";
        private const string PARM_CONFERENCEADDRESS = "CONFERENCEADDRESS";
        private const string PARM_CONFERENCEORGANISER = "CONFERENCEORGANISER";
        private const string PARM_CO_SPONSOR = "CO_SPONSOR";
        private const string PARM_SPONSOR = "SPONSOR";
        private const string PARM_INTERNALRESOURCESFLAG = "INTERNALRESOURCESFLAG";
        ...
        /// <returns></returns>
        public bool Add(ConferencePaperInfo item)
        {
            if (item == null)
            {
                return false;
            ...
            if (!string.IsNullOrEmpty(item.ConferenceName))
            {
                paramList.Add(PARM_CONFERENCENAME);
                paramList.Add(item.ConferenceName);
            }
            if (!string.IsNullOrEmpty(item.ConferenceTime))
            {
                paramList.Add(PARM_CONFERENCETIME);
                paramList.Add(item.ConferenceTime);
            }
            if (!string.IsNullOrEmpty(item.ConferenceAddress))
            {
                paramList.Add(PARM_CONFERENCEADDRESS);
                paramList.Add(item.ConferenceAddress);
            }
            if (!string.IsNullOrEmpty(item.ConferenceOrganiser))
            {
                paramList.Add(PARM_CONFERENCEORGANISER);
                paramList.Add(item.ConferenceOrganiser);
            }
            if (!string.IsNullOrEmpty(item.Co_sponsor))
            {
                paramList.Add(PARM_CO_SPONSOR);
                paramList.Add(item.Co_sponsor);
            }
            if (!string.IsNullOrEmpty(item.sponsor))
            {
                paramList.Add(PARM_SPONSOR);
                paramList.Add(item.sponsor);
            }
            paramList.Add(PARM_INTERNALRESOURCESFLAG);
            paramList.Add(item.InternalResourcesFlag.ToString());
            ...
            paramList.Add(item.PrintNUM.ToString());
        ...
        /// <param name="item"></param>
        /// <returns></returns>
        public bool Update(ConferencePaperInfo item)
        {
            if (item == null)
            {
                return false;
        ...
        /// <param name="doi"></param>
        /// <returns></returns>
        public ConferencePaperInfo GetItem(string doi)
        {
            ...
            try
            {
                ConferencePaperInfo entry = new ConferencePaperInfo();
                #region 判断字段并赋值
                entry.ConferenceName = rs.GetValue(PARM_CONFERENCENAME) ?? "";
                entry.ConferenceTime = rs.GetValue(PARM_CONFERENCETIME) ?? "";
                entry.ConferenceAddress = rs.GetValue(PARM_CONFERENCEADDRESS) ?? "";
                entry.ConferenceOrganiser = rs.GetValue(PARM_CONFERENCEORGANISER) ?? "";
                entry.Co_sponsor = rs.GetValue(PARM_CO_SPONSOR) ?? "";
                entry.sponsor = rs.GetValue(PARM_SPONSOR) ?? "";
                entry.InternalResourcesFlag = StructTrans.TransNum(rs.GetValue(PARM_INTERNALRESOURCESFLAG));
                ...
                //entry.EName = rs.GetValue(PARM_ENAME) ?? "";
        ...
        /// <returns></returns>
        public List<ConferencePaperInfo> GetList(string sqlWhere, int pageNo, int pageCount, out int recordCount, bool IsAll)
        {
            recordCount = 0;
            ...
            try
            {
                List<ConferencePaperInfo> entryList = new List<ConferencePaperInfo>();
                ConferencePaperInfo entry = null;
                for (int i = 0; i < pageCount; i++)
                {
                    entry = new ConferencePaperInfo();
                    #region 判断字段并赋值
                    entry.ConferenceName = rs.GetValue(PARM_CONFERENCENAME) ?? "";
                    entry.ConferenceTime = rs.GetValue(PARM_CONFERENCETIME) ?? "";
                    entry.ConferenceAddress = rs.GetValue(PARM_CONFERENCEADDRESS) ?? "";
        ...
        public bool DeleteByWhere(string strWhere)
        {
            ...
            int record = 0;
            List<ConferencePaperInfo> list = GetList(strWhere, 1, 1000, out record, false);
            if (list == null)
            {
