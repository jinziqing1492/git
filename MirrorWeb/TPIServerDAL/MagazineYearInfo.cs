...
{
    public class MagazineYearInfo : IMagazineYearInfo
    {
        private static string TABLE_NAME = ConfigurationManager.AppSettings["MagazineYearInfo"];
        #region IArticle 字段
        ...
        private const string PARM_PRODUCTOR = "PRODUCTOR";
        private const string PARM_EDITOR = "EDITOR";
        private const string PARM_SYS_FLD_INFOXML = "SYS_FLD_INFOXML";
        ...
        /// <returns></returns>
        public bool Add(Model.MagazineYearInfo item)
        {
            if (item == null)
            {
                return false;
            ...
            if (!string.IsNullOrEmpty(item.Productor))
            {
                paramList.Add(PARM_PRODUCTOR);
                paramList.Add(item.Productor);
            }
            if (!string.IsNullOrEmpty(item.Editor))
            {
                paramList.Add(PARM_EDITOR);
                paramList.Add(item.Editor);
            }
            if (!string.IsNullOrEmpty(item.Sys_fld_infoxml))
            {
                paramList.Add(PARM_SYS_FLD_INFOXML);
                paramList.Add(item.Sys_fld_infoxml);
            }
        ...
        /// <param name="item"></param>
        /// <returns></returns>
        public bool Update(Model.MagazineYearInfo item)
        {
            if (item == null)
            {
                return false;
        ...
        /// <param name="doi"></param>
        /// <returns></returns>
        public Model.MagazineYearInfo GetItem(string doi)
        {
            ...
            try
            {
                Model.MagazineYearInfo entry = new Model.MagazineYearInfo();
                #region 判断字段并赋值
                ...
                entry.Productor = rs.GetValue(PARM_PRODUCTOR) ?? "";
                entry.Editor = rs.GetValue(PARM_EDITOR) ?? "";
                entry.Sys_fld_infoxml = rs.GetValue(PARM_SYS_FLD_INFOXML) ?? "";
        ...
        /// <returns></returns>
        public List<Model.MagazineYearInfo> GetList(string sqlWhere, int pageNo, int pageCount, out int recordCount, bool IsAll)
        {
            recordCount = 0;
            ...
            try
            {
                List<Model.MagazineYearInfo> entryList = new List<Model.MagazineYearInfo>();
                Model.MagazineYearInfo entry = null;
                for (int i = 0; i < pageCount; i++)
                {
                    entry = new Model.MagazineYearInfo();
                    #region 判断字段并赋值
                    entry.BASEID = rs.GetValue(PARM_BASEID) ?? "";
                    entry.CNAME = rs.GetValue(PARM_CNAME) ?? "";
                    entry.YEAR = StructTrans.TransNum(rs.GetValue(PARM_YEAR));
        ...
        public bool DeleteByWhere(string strWhere)
        {
            ...
            int record=0;
            List<Model.MagazineYearInfo> list = GetList(strWhere, 1, 1000, out record, false);
            if (list == null)
            {
            ...
            foreach (Model.MagazineYearInfo info in list)
            {
                //删除图片
                ...
                //删除杂志文章信息
                IsSuccess = magazinearticle.DeleteByWhere("ParentDoi='" + info.SYS_FLD_DOI + "'");
                if (!IsSuccess)
                {
                    return false;
                }
