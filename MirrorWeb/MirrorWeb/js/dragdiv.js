...
$(function () {
    if ("undefined" != typeof dragMode && dragMode == "Default") {
        //默认设置
        $('.skAdvancsc').dragDrop();
        return;
    }

    //获取高级检索div的width和height
    var offset = $('.zy-enclosure').offset();
    var enclosurewidth = $('.zy-enclosure').width();
    var enclosureheight = $('.zy-enclosure').height();
    //限定区域，有回调函数。
    $('.zy-enclosure').dragDrop({
        focuEle: $('.skenclosure-tit'),
        fixarea: [5, $('body').width() - (enclosurewidth + 10), 10, $('body').height() - enclosureheight - 5],
        callback: function (params) {
            //$('#span1').text('X:' + params.moveX + ' Y:' + params.moveY);
            //alert("drag  " + 'X:' + params.moveX + ' Y:' + params.moveY);
        } 
    });
});