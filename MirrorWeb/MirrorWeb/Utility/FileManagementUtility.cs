        ...
        public static BaseModel GetResObjByDoi(DataBaseType dbtype, string doi)
        {
            ...
            switch (dbtype)
            {
                ...
                case DataBaseType.JOURNALYEAR:
                    ...
                    JournalYearInfo yearInfo = journalyear.GetItem(doi);
                    if (yearInfo != null)
                    {
                        bm.Name = yearInfo.CNAME + yearInfo.YEAR + "年" + yearInfo.ISSUE;
                    }
                    if (bm == null)
                    {
                        JournalYear englishBll = new JournalYear("english");
                        bm = englishBll.GetItem(doi);
                        JournalYearInfo englishInfo = englishBll.GetItem(doi);
                        if (englishInfo != null)
                        {
                            bm.Name = englishInfo.CNAME + englishInfo.YEAR + "年" + englishInfo.ISSUE + "期";
                        }
                    }
                    break;
                ...
                case DataBaseType.ENGLISHRES:
                    JournalYear englishyear = new JournalYear("english");
                    bm = englishyear.GetItem(doi);
                    break;
                case DataBaseType.STUDYRES:
                    JournalYear studyyear = new JournalYear("study");
                    bm = studyyear.GetItem(doi);
                    break;
                case DataBaseType.OWNERRES:
                    JournalYear owneryear = new JournalYear("owner");
                    bm = owneryear.GetItem(doi);
                    break;
                default:
                    break;
            }
