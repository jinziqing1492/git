...
namespace DRMS.TPIServerDAL
{
    ...
    {
        private static string TABLE_NAME = ConfigurationManager.AppSettings["Audio"];
        #region IArticle 字段
        ...
        private const string PARM_ISSUEDATE = "DATEISSUED";
        private const string PARM_AUDIOTYPE = "AUDIOTYPE";
        private const string PARM_AUDIOSIZE = "AUDIOSIZE";
        private const string PARM_AUDIOTIME = "AUDIOTIME";
        ...
        /// <returns></returns>
        public bool Add(AudioInfo item)
        {
            if (item == null)
            {
                return false;
            ...
            if (!string.IsNullOrEmpty(item.AudioType))
            {
                paramList.Add(PARM_AUDIOTYPE);
                paramList.Add(item.AudioType);
            }
            if (!string.IsNullOrEmpty(item.AudioSize))
            {
                paramList.Add(PARM_AUDIOSIZE);
                paramList.Add(item.AudioSize);
            }
            if (!string.IsNullOrEmpty(item.AudioTime))
            {
                paramList.Add(PARM_AUDIOTIME);
                paramList.Add(item.AudioTime);
            }
        ...
        /// <param name="item"></param>
        /// <returns></returns>
        public bool Update(AudioInfo item)
        {
            if (item == null)
            {
                return false;
        ...
        /// <param name="doi"></param>
        /// <returns></returns>
        public AudioInfo GetItem(string doi)
        {
            ...
            try
            {
                ...
                entry.DateIssued = StructTrans.TransDate(rs.GetValue(PARM_ISSUEDATE));
                entry.AudioType = rs.GetValue(PARM_AUDIOTYPE) ?? "";
                entry.AudioSize = rs.GetValue(PARM_AUDIOSIZE);
                entry.AudioTime = rs.GetValue(PARM_AUDIOTIME);
        ...
        /// <returns></returns>
        public List<AudioInfo> GetList(string sqlWhere, int pageNo, int pageCount, out int recordCount, bool IsAll)
        {
            recordCount = 0;
            ...
            try
            {
                List<AudioInfo> entryList = new List<AudioInfo>();
                AudioInfo entry = null;
                for (int i = 0; i < pageCount; i++)
                {
                    ...
                    entry.Description = rs.GetValue(PARM_DESCRIPTION) ?? "";
                    entry.Themeword = rs.GetValue(PARM_THEMEWORD) ?? "";
                    entry.Type = StructTrans.TransNum(PARM_TYPE);
                    entry.Note = rs.GetValue(PARM_NOTE) ?? "";
                    entry.Language = rs.GetValue(PARM_LANGUAGE) ?? "";
                    entry.Source = rs.GetValue(PARM_SOURCE) ?? "";
                    entry.DateIssued = StructTrans.TransDate(rs.GetValue(PARM_ISSUEDATE));
