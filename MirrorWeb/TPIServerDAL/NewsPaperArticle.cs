...
{
    public class NewsPaperArticle : INewsPaperArticle
    {
        private static string TABLE_NAME = ConfigurationManager.AppSettings["NewsPaperArticle"];
        #region IArticle 字段
        ...
        /// <returns></returns>
        public bool Add(NewsPaperArticleInfo item)
        {
            if (item == null)
            {
                return false;
            ...
            if (!string.IsNullOrEmpty(item.LayoutName))
            {
                paramList.Add(PARM_LAYOUTNAME);
                paramList.Add(item.LayoutName);
            }
            ...
            {
                paramList.Add(PARM_MAPSTR);
                paramList.Add(item.Mapstr);
            }
        ...
        /// <param name="item"></param>
        /// <returns></returns>
        public bool Update(NewsPaperArticleInfo item)
        {
            if (item == null)
            {
                return false;
        ...
        /// <param name="doi"></param>
        /// <returns></returns>
        public NewsPaperArticleInfo GetItem(string doi)
        {
            ...
            try
            {
                NewsPaperArticleInfo entry = new NewsPaperArticleInfo();
                #region 判断字段并赋值
        ...
        /// <returns></returns>
        public List<NewsPaperArticleInfo> GetList(string sqlWhere, int pageNo, int pageCount, out int recordCount, bool IsAll)
        {
            recordCount = 0;
            ...
            try
            {
                List<NewsPaperArticleInfo> entryList = new List<NewsPaperArticleInfo>();
                NewsPaperArticleInfo entry = null;
                for (int i = 0; i < pageCount; i++)
                {
                    entry = new NewsPaperArticleInfo();
                    #region 判断字段并赋值
                    entry.Title = rs.GetValue(PARM_TITLE) ?? "";
                    entry.SubTitle = rs.GetValue(PARM_SUBTITLE) ?? "";
                    entry.FirstAuthor = rs.GetValue(PARM_FIRSTAUTHOR) ?? "";
        ...
        public bool DeleteByWhere(string strWhere)
        {
            ...
            int record = 0;
            List<NewsPaperArticleInfo> list = GetList(strWhere, 1, 1000, out record, false);
            if (list == null)
            {
            ...
            Pic picture = new Pic();
            foreach (NewsPaperArticleInfo info in list)
            {
                //删除图片信息
