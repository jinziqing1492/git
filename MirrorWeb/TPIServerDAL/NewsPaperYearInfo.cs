...
{
    public class NewsPaperYearInfo : INewsPaperYearInfo
    {
        private static string TABLE_NAME = ConfigurationManager.AppSettings["NewsPaperYearInfo"];
        #region IArticle 字段
        ...
        /// <returns></returns>
        public bool Add(Model.NewsPaperYearInfo item)
        {
            if (item == null)
            {
                return false;
        ...
        /// <param name="item"></param>
        /// <returns></returns>
        public bool Update(Model.NewsPaperYearInfo item)
        {
            if (item == null)
            {
                return false;
        ...
        /// <returns></returns>
        public Model.NewsPaperYearInfo GetItem(string thname)
        {
            if (string.IsNullOrEmpty(thname))
            {
                return null;
            }
            string sqlQuery = string.Format("SELECT * FROM {0} WHERE {1} = '{2}'", TABLE_NAME, PARM_SYS_FLD_DOI, thname);
            ...
            try
            {
                Model.NewsPaperYearInfo entry = new Model.NewsPaperYearInfo();
                #region 判断字段并赋值
        ...
        /// <returns></returns>
        public List<Model.NewsPaperYearInfo> GetList(string sqlWhere, int pageNo, int pageCount, out int recordCount, bool IsAll)
        {
            recordCount = 0;
            ...
            try
            {
                List<Model.NewsPaperYearInfo> entryList = new List<Model.NewsPaperYearInfo>();
                Model.NewsPaperYearInfo entry = null;
                for (int i = 0; i < pageCount; i++)
                {
                    entry = new Model.NewsPaperYearInfo();
                    #region 判断字段并赋值
                    entry.BASEID = rs.GetValue(PARM_BASEID) ?? "";
                    entry.CNAME = rs.GetValue(PARM_CNAME) ?? "";
                    entry.YEAR = StructTrans.TransNum(rs.GetValue(PARM_YEAR));
        ...
        public bool DeleteByWhere(string strWhere)
        {
            ...
            int record = 0;
            List<Model.NewsPaperYearInfo> list = GetList(strWhere, 1, 1000, out record, false);
            if (list == null)
            {
            ...
            NewsPaperArticle newpaperarticle = new NewsPaperArticle();
            foreach (Model.NewsPaperYearInfo info in list)
            {
                //删除图片信息
                ...
                //删除附件信息
                IsSuccess = attch.DeleteByWhere("ParentDoi='" + info.SYS_FLD_DOI + "'");
                if (!IsSuccess)
                {
                    return false;
                }
                ...
                //删除报纸文章信息
                IsSuccess = newpaperarticle.DeleteByWhere("ParentDoi='" + info.SYS_FLD_DOI + "'");
                if (!IsSuccess)
                {
                    return false;
                }
