...
{
    public class MagazineInfo : IMagazineInfo
    {
        private static string TABLE_NAME = ConfigurationManager.AppSettings["MagazineInfo"];
        #region IArticle 字段
        ...
        /// <returns></returns>
        public bool Add(Model.MagazineInfo item)
        {
            if (item == null)
            {
                return false;
        ...
        /// <param name="item"></param>
        /// <returns></returns>
        public bool Update(Model.MagazineInfo item)
        {
            if (item == null)
            {
                return false;
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public Model.MagazineInfo GetItem(string id)
        {
            ...
            try
            {
                Model.MagazineInfo entry = new Model.MagazineInfo();
                #region 判断字段并赋值
        ...
        /// <returns></returns>
        public List<Model.MagazineInfo> GetList(string sqlWhere, int pageNo, int pageCount, out int recordCount, bool IsAll)
        {
            recordCount = 0;
            ...
            try
            {
                List<Model.MagazineInfo> entryList = new List<Model.MagazineInfo>();
                Model.MagazineInfo entry = null;
                for (int i = 0; i < pageCount; i++)
                {
                    entry = new Model.MagazineInfo();
                    #region 判断字段并赋值
                    entry.BASEID = rs.GetValue(PARM_BASEID) ?? "";
                    entry.CNAME = rs.GetValue(PARM_CNAME) ?? "";
                    entry.Description = rs.GetValue(PARM_DESCRIPTION) ?? "";
        ...
        public bool DeleteByWhere(string strWhere)
        {
            ...
            int record;
            List<Model.MagazineInfo> list = GetList(strWhere, 1, 1000, out record, false);
            if (list == null)
            {
                return true;//list为空时，表示表中没有相关数据，在KBASE中，删除不存在的记录返回为空
            }

            MagazineYearInfo magazineyear = new MagazineYearInfo();
            MagazineArticle magazinearticle = new MagazineArticle();
            foreach (Model.MagazineInfo info in list)
            {
                //删除杂志文章
                bool IsSuccess = magazinearticle.DeleteByWhere("BaseId='" + info.BASEID + "'");
                if (!IsSuccess)
                {
                    return false;
                }
                ...
                //删除杂志年信息
                IsSuccess = magazineyear.DeleteByWhere("BaseId='" + info.BASEID + "'");
                if (!IsSuccess)
                {
                    return false;
                }
        ...
        {
            string strSet = string.Format("UPDATE {0} SET SYS_FLD_CHECK_DATE='{1}',ISONLINE='{2}' WHERE BASEID='{3}'", TABLE_NAME, dateTime, isOnLine, id);
            return TPIHelper.ExecSql(strSet);
        }
