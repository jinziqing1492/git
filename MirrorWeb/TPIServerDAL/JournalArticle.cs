...
{
    public class JournalArticle : IJournalArticle
    {
        /// <summary>
        /// 设置表名
        /// </summary>
        ...
        public void SetTableName(string type)
        {
            switch (type)
            {
                case "journal": TABLE_NAME = ConfigurationManager.AppSettings["JournalArticle"];
                    break;
                case "english": TABLE_NAME = ConfigurationManager.AppSettings["EnglishArticle"];
                    break;
                case "study": TABLE_NAME = ConfigurationManager.AppSettings["Book1"];
                    break;
                case "owner": TABLE_NAME = ConfigurationManager.AppSettings["OwnerResArticle"];
                    break;
                case "ownerandjournalarticle": TABLE_NAME = ConfigurationManager.AppSettings["ownerandjournalarticle"];
                    break;
                default: TABLE_NAME = ConfigurationManager.AppSettings["JournalArticle"];
                    break;
            }
        }

        private string TABLE_NAME = ConfigurationManager.AppSettings["JournalArticle"];
        #region IArticle 字段
        ...
        private const string PARM_ENAME = "ENAME";
        ...
        /// <returns></returns>
        public bool Add(JournalArticleInfo item)
        {
            if (item == null)
            {
                return false;
            ...
            {
                paramList.Add(PARM_ENAME);
                paramList.Add(item.EName);
            }
            ...
            if (!string.IsNullOrEmpty(PARM_SYS_FLD_PARAXML))
            {
                paramList.Add(PARM_SYS_FLD_PARAXML);
                paramList.Add(item.SYS_FLD_PARAXML);
            }
        ...
        /// <param name="item"></param>
        /// <returns></returns>
        public bool Update(JournalArticleInfo item)
        {
            if (item == null)
            {
                return false;
        ...
        /// <param name="doi"></param>
        /// <returns></returns>
        public JournalArticleInfo GetItem(string doi)
        {
            ...
            try
            {
                JournalArticleInfo entry = new JournalArticleInfo();
                #region 判断字段并赋值
        ...
        /// <returns></returns>
        public List<JournalArticleInfo> GetList(string sqlWhere, int pageNo, int pageCount, out int recordCount, bool IsAll)
        {
            recordCount = 0;
            ...
            try
            {
                List<JournalArticleInfo> entryList = new List<JournalArticleInfo>();
                JournalArticleInfo entry = null;
                for (int i = 0; i < pageCount; i++)
                {
                    entry = new JournalArticleInfo();
                    #region 判断字段并赋值
                    entry.Name = rs.GetValue(PARM_NAME) ?? "";
                    string title = entry.Name;
                    entry.Note = Util.SubNote(ref title);
        ...
        public bool DeleteByWhere(string strWhere)
        {
            ...
            int record=0;
            List<JournalArticleInfo> list = GetList(strWhere, 1, 1000, out record, false);
            if (list == null)
            {
            ...
            if (list != null)
            {
                ...
                {
                    if (!p.DeleteByWhere("SYS_FLD_ChapterDoi='" + info.SYS_FLD_DOI + "'"))
                    {
                        return false;
                    }
                }
