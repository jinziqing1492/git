...
{
    public class ToolBook : IToolBook
    {
        private static string TABLE_NAME = ConfigurationManager.AppSettings["ToolBook"];
        #region IArticle 字段
        ...
        /// <returns></returns>
        public bool Add(ToolBookInfo item)
        {
            if (item == null)
            {
                return false;
            ...
            if (!string.IsNullOrEmpty(item.SYS_FLD_ABSTRACT))
            {
                paramList.Add(PARM_SYS_FLD_ABSTRACT);
                paramList.Add(item.SYS_FLD_ABSTRACT);
            }
        ...
        /// <param name="item"></param>
        /// <returns></returns>
        public bool Update(ToolBookInfo item)
        {
            if (item == null)
            {
                return false;
        ...
        /// <param name="doi"></param>
        /// <returns></returns>
        public ToolBookInfo GetItem(string doi)
        {
            ...
            try
            {
                ToolBookInfo entry = new ToolBookInfo();
                #region 判断字段并赋值
        ...
        /// <returns></returns>
        public List<ToolBookInfo> GetList(string sqlWhere, int pageNo, int pageCount, out int recordCount, bool IsAll)
        {
            recordCount = 0;
            ...
            try
            {
                List<ToolBookInfo> entryList = new List<ToolBookInfo>();
                ToolBookInfo entry = null;
                for (int i = 0; i < pageCount; i++)
                {
                    entry = new ToolBookInfo();
                    #region 判断字段并赋值
                    entry.Name = rs.GetValue(PARM_NAME) ?? "";
                    entry.ENName = rs.GetValue(PARM_ENNAME) ?? "";
                    entry.Author = rs.GetValue(PARM_AUTHOR) ?? "";
        ...
        public bool DeleteByWhere(string strWhere)
        {
            ...
            int record = 0;
            List<ToolBookInfo> list = GetList(strWhere, 1, 1000, out record, false);
            if (list == null)
            {
            ...
            foreach (ToolBookInfo info in list)
            {
                ...
                string strWhere1 = "parenturlid='" + info.SYS_FLD_DOI + "'";
                //获取词条，用以删除词条内容记录
                List<TerminologyInfo> termlist = dal.GetList(strWhere1, 1, 1000, out record, false);
